...
class Git_Snake:
    ...
    def movement_IA(self, position):
        ...
        c = "p"
        if self.aling == 0:
            self.aling = random.randint(1, 2)
        if self.valid:
            self.hit_left = self.hit_right = self.hit_you = False
        if self.pos[2] in position:
            position.remove(self.pos[2])
